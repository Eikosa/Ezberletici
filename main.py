...
def calismaya_basla():
    ...
    def baslat():
        ...
        window.ogrendimButton.setEnabled(True)
        window.tekrarlaButton.setEnabled(True)
...
def cevabi_goster():
    ...
    window.ogrendimButton.setText("Öğrendim")
    window.tekrarlaButton.setText("Tekrarla")

def learned_card():
    if window.ogrendimButton.text() == "Cevabı Göster":
        cevabi_goster()
        return
    global soru_cevap
...
def get_next_card():
    ...
    window.ogrendimButton.setText("Cevabı Göster")
    window.tekrarlaButton.setText("Cevabı Göster")
...
def dosya_sec():
    ...
    try:
        with open("cards/" + window.comboBox.currentText(), "r", encoding="utf-8") as f:
